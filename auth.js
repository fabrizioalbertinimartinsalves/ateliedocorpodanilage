...
async function _sessaoAdminPronta(callback) {
  // Mostra UI imediatamente — sem esperar dbLoad
  document.getElementById('tela-login')?.setAttribute('style', 'display:none');
  // Remove style inline para deixar o CSS decidir (flex no desktop, block no mobile)
  ...
  // Chama callback imediatamente para renderizar a página
  callback?.(_usuarioAtual);
  // Carrega dados em segundo plano sem bloquear
  dbLoad(true).catch(e => console.warn('[dbLoad background]', e));
}
...
function _mostrarLogin() {
  ...
  const wrapper = document.querySelector('.wrapper');
  if (wrapper) { wrapper.removeAttribute('style'); wrapper.style.display = 'none'; }
}
